...
1. **Discovery of templates** — **corrected 2026-08-19, verified against `home-assistant/frontend` source**: `hass.lovelace` is never actually populated on the `hass` object a plain Lovelace card element receives (it's component-local state in `ha-panel-lovelace.ts`, only ever passed to view/editor elements, never merged onto `hass`). The `lovelace/config` websocket call is therefore the *only* real path, not a fallback — and it requires an explicit `url_path` (the current dashboard's, resolved from `window.location.pathname` cross-checked against `hass.panels`), since omitting it always fetches the instance's built-in default dashboard, not whichever one is being viewed.
