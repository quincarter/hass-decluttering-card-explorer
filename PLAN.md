...
>
> **This plan was rewritten after a direction change.** The original plan built a _standalone read-only explorer card_. The new goal (decided with the user) is to **feed decluttering templates into Home Assistant's native "Add Card" selector** so that, while building a dashboard, every template shows up as a selectable card with a live preview and inserts pre-filled.
...
`registerAll`'s reconciliation only releases _tag ownership_ so a colliding survivor
...
**Goal:** A card element that, once loaded on a dashboard, reads the dashboard's `decluttering_templates` and **registers each template into HA's native Add Card picker** (`window.customCards`) via a dynamically-defined `decluttering-card-*` custom element (no `hui-card-` prefix — see the corrected Architecture section below) whose `getStubConfig()` returns a fully pre-filled `custom:decluttering-card`. Clicking a template in the native picker shows a live preview and inserts the pre-filled card. Optionally it can also render a compact in-dashboard list/explorer as a bonus.

**Architecture (grounded in `home-assistant/frontend` `dev`):**

1. **Discovery of templates** — **corrected 2026-08-19, verified against `home-assistant/frontend` source**: `hass.lovelace` is never actually populated on the `hass` object a plain Lovelace card element receives (it's component-local state in `ha-panel-lovelace.ts`, only ever passed to view/editor elements, never merged onto `hass`). The `lovelace/config` websocket call is therefore the _only_ real path, not a fallback — and it requires an explicit `url_path` (the current dashboard's, resolved from `window.location.pathname` cross-checked against `hass.panels`), since omitting it always fetches the instance's built-in default dashboard, not whichever one is being viewed.
2. **Per-template synthetic element** — for each template `T`, define a HTMLElement subclass (Lit `LitElement`) at tag `decluttering-card-<safe(T)>` (**corrected 2026-08-19**: no `hui-card-` prefix — that convention only applies to HA's own built-in card types via a completely different `hui-${type}-card` resolution path; custom cards are resolved as `customElements.get(stripCustomPrefix(config.type))` directly, so the defined tag must exactly equal the `type` string registered in `window.customCards`) with:
...
3. **Register into the native picker** — push `{ type: "decluttering-card-<safe(T)>", name, description, preview: true }` into the global `window.customCards` array. The picker's `_loadCards()` maps every `window.customCards` entry into a selectable row; clicking it calls `getCardStubConfig(type)` → `getCardElementClass(type)` → `tryCreateLovelaceElement` which strips the `custom:` prefix and resolves `decluttering-card-<safe(T)>` (no `hui-card-` prefix) to **our** element. So the inserted config is exactly our stub, pre-filled. Note: the _live preview thumbnail_ shown in the picker's "By card" search results does **not** render our element at all — it calls our `getStubConfig()`, whose returned `type: "custom:decluttering-card"` overwrites the picker's own draft config, so the thumbnail actually renders the real `decluttering-card`. Our registered element's `render()` is effectively dead code for that path; it only matters if something resolves `custom:decluttering-card-<safe(T)>` directly.
...
- **Picker re-render timing:** the native picker caches its card list in `_loadCards()` which runs on attach. Adding entries to `window.customCards` after the picker is already open won't refresh it until reopened. This is fine — the loader runs when the dashboard loads, before the user opens Add Card. If a template is added mid-session, the `lovelace_updated` re-registration updates `window.customCards`, but the _open_ picker won't live-refresh (HA limitation). Acceptable for v1; note it.
